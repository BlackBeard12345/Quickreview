...
<div class="row">
    ...
    <div class="col-12">
        <form class=" mt-5" id="addReviewForm" style="display: none" method="POST" action="{{ route('user.reviews.store', $film->id) }}">
            @csrf
                ...
                <div class="card-footer text-muted">
                    <div >
                        @if (Auth::id() == $review->user->id)
                                <a href="{{ route('user.reviews.edit', $film->id) }}" class="btn btn-warning float-end m-1 text-dark">
                                    Edytuj
                                </a>
                                <a href="{{ route('user.reviews.delete', $film->id) }}" class="btn btn-danger float-end m-1 text-dark">
                                    Usuń
                                </a>
                        @endif
                        <a href="{{ route('user.reviews.show', $review->id) }}" class="btn btn-info float-end m-1 text-dark" >
                            Zobacz
                        </a>
                    </div>
